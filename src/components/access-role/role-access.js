(function () {
  ...
  function hasAdminAccess(sessionUser) {
    ...
    if (sessionUser.isAdmin === true || sessionUser.role === 'admin') {
      return true;
    }
  ...
  function renderWelcome(sessionUser) {
    if (!welcome || !sessionUser || !sessionUser.email) {
      return;
    }
  ...
  function createAdminCard() {
    ...
    card.innerHTML =
      ...
      '</ul>' +
      '<button type="button" class="role-card-action" onclick="window.location.href=\'../profile-admin/index.html\'">Continuar como administrador</button>';
